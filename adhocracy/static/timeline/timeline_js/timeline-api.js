    ...
    var loadMe = function() {
        ...
        try {
            ...
            */
            ...
            // fabio: replace of above            
            /*
             *  Include non-localized files
             */
            
            var timelineJavascriptFiles = new Array();
            var addToJavascriptFileList = function (urlPrefix, filenames){for (var j=0; j<filenames.length; j++) timelineJavascriptFiles.push(urlPrefix+filenames[j]);}
            
            if (!isCompiled) {
                if (bundle) {
                    //includeJavascriptFiles(Timeline.urlPrefix, [ "timeline-bundle.js" ]);
                    addToJavascriptFileList(Timeline.urlPrefix, [ "timeline-bundle.js" ]);
                    includeCssFiles(Timeline.urlPrefix, [ "timeline-bundle.css" ]);
                } else {
                    //includeJavascriptFiles(Timeline.urlPrefix + "scripts/", javascriptFiles);
                    addToJavascriptFileList(Timeline.urlPrefix + "scripts/", javascriptFiles);
                    includeCssFiles(Timeline.urlPrefix + "styles/", cssFiles);
                }
            }
            ...
            if (!isCompiled) {
                for (var l = 0; l < supportedLocales.length; l++) {
                    ...
                    if (loadLocale[locale]) {
                        //includeJavascriptFiles(Timeline.urlPrefix + "scripts/l10n/" + locale + "/", localizedJavascriptFiles);
                        addToJavascriptFileList(Timeline.urlPrefix + "scripts/l10n/" + locale + "/", localizedJavascriptFiles);
                        includeCssFiles(Timeline.urlPrefix + "styles/l10n/" + locale + "/", localizedCssFiles);
                    }
                }
            }
            includeJavascriptFiles('',timelineJavascriptFiles);
            // /fabio: replace of above            
